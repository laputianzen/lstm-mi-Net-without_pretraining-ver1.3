...
def multi_class_read(datadir,file_str,num_inst,dataset):
    instIdx = np.insert(np.cumsum(num_inst),0,0)
    newIdx = 0
    for k in range(len(dataset.k)):
        for c in range(len(dataset.C5k_CLASS[k])):
            tactic = dataset.tacticName[dataset.C5k_CLASS[k][c]]
            fileName= file_str.format(tactic,dataset.k[k])
            data_X, data_Y, data_KIlabel = read(datadir,fileName)
            data_KPlabel = Inst2Player(data_KIlabel,dataset.playerMap[k])
            if k == 0 and c == 0:
                multi_X = np.zeros([data_X.shape[0],np.sum(num_inst),data_X.shape[2]])
                multi_Y = np.zeros([data_Y.shape[0],len(dataset.tacticName)]) 
